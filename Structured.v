...
Fixpoint remove_one (v:nat) (s:bag) :=
  match s with
  | nil => nil
  | x :: xs => if beq_nat v x then xs else x :: (remove_one v xs)
  end.
...
Fixpoint remove_all (v:nat) (s:bag) :=
  match s with
  | nil => nil
  | x :: xs => if beq_nat v x then remove_all v xs else x::remove_all v xs
  end.
...
Theorem beq_nat_eq : forall n : nat, beq_nat n n = true.
Proof.
  intros n.
...
Proof.
  ...
  induction l as [| n l' IHl'].
  - reflexivity.
  - simpl. rewrite -> app_length. rewrite -> IHl'. rewrite -> plus_comm. simpl. reflexivity.
Qed.
...
Theorem app_assoc4: forall l1 l2 l3 l4,
...
Fixpoint beq_natlist (l1 l2 : natlist) : bool :=
  match l1,l2 with
  | nil,nil => true
  | hd1::tl1,hd2::tl2 => beq_nat hd1 hd2 && beq_natlist tl1 tl2
  | _,_ => false
  end.
...
Proof.
  ...
  induction l as [| n l' IHl'].
  - reflexivity.
  - simpl. rewrite -> beq_nat_eq. simpl. rewrite -> IHl'. reflexivity.
Qed.
...
Example test_hd_error1 : hd_error [1] = Some 1.
Proof. reflexivity. Qed.

Example test_hd_error2: hd_error [] = None.
Proof. reflexivity. Qed.
...
Proof. intros. destruct x. simpl. rewrite -> NatList.beq_nat_eq. reflexivity. Qed.
